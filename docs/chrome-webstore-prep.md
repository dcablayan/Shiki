...
| Listing assets | Existing 16/32/48/128 icons present. No Web Store screenshots or promo tiles in repo. | Store listing needs at least one screenshot and a 128x128 icon; dashboard may request promo assets. | Need screenshots/promo images before final submission. |
...
1. Will launch be free in all regions, or restricted by country/tester group?
2. Should reviewers get a paid ChatGPT/Claude/Gemini test account, or should test instructions say core functionality can be tested with any reviewer-owned supported-provider account?
